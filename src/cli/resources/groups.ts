  ...
  customOperations: {
    ...
    'config add-mcp-server': {
      ...
      description:
        ...
        'Stdio: --id <group-id> --name <name> --command <cmd> [--args <json-array>] [--env <json-object>]. ' +
        'HTTP: --id <group-id> --name <name> --transport http --url <url> [--headers <json-object>].',
      handler: async (args) => {
        const id = args.id as string;
        if (!id) throw new Error('--id is required');
        ...
        if (transport === 'http' || transport === 'https') {
          ...
          if (transport === 'https') url = url.replace(/^http:/, 'https:');
          servers[name] = {
            type: 'http',
            url,
